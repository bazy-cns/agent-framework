...
| G. real source `DefaultHttpRequestHandler` / `IHttpRequestHandler` 是否通过 fake handler 构造 request | PASS: real source files linked from the repository constructed the expected request through a fake `HttpMessageHandler`. |
| H. real source metadata-looking URL 是否仍只进入 fake handler | PASS: real source `DefaultHttpRequestHandler` sent the metadata-looking URL only to the fake handler; `NetworkSendCount == 0`. |
| I. real source redirect 是否仍只是 fake 302 | PASS: real source fake redirect remained a fake 302; no real redirect/network follow was performed. |
...
- The real-source integration checks compile the repository source files for `IHttpRequestHandler`, `HttpRequestInfo`, `HttpRequestResult`, and `DefaultHttpRequestHandler` directly into the PoC project, while still using only fake `HttpMessageHandler` transport.
