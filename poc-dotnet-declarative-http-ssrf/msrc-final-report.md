...
- Primary types/files reviewed:
  - `DeclarativeWorkflowOptions.HttpRequestHandler` in `dotnet/src/Microsoft.Agents.AI.Workflows.Declarative/DeclarativeWorkflowOptions.cs`.
  - `HttpRequestExecutor` in `dotnet/src/Microsoft.Agents.AI.Workflows.Declarative/ObjectModel/HttpRequestExecutor.cs`.
...
1. `DeclarativeWorkflowOptions.HttpRequestHandler` is the host-supplied handler setting for declarative HTTP actions. If it is not set, HTTP request actions are documented to fail.
...
E. A fake `HttpMessageHandler` captures the request constructed by the default-handler model.
