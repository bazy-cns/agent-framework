...
- `HttpSsrSinkHarness.cs` — console harness with A-F model checks.
- `RealSourceIntegrationHarness.cs` — no-network checks that compile the repository source files for the real `IHttpRequestHandler` / `HttpRequestInfo` / `HttpRequestResult` / `DefaultHttpRequestHandler` types and exercise them through a fake `HttpMessageHandler`.
...
G-I. Real-source integration checks compile the repository source files for the real declarative HTTP handler types and verify request construction, metadata-string-only handling, and fake redirect behavior through fake `HttpMessageHandler` transport only.
