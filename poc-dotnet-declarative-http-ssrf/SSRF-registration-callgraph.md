...
Declarative HTTP execution depends on a host-provided `IHttpRequestHandler`. The relevant public contract in the reviewed source is `IHttpRequestHandler`, which defines the execution boundary for requests emitted by declarative workflow `HttpRequestAction` actions.
